       ...
       // return inflater.inflate(R.layout.fragment_cart, container, false);
        ...
        if(cart != null) {
            if(cart.size() == 0){
                ...
                tv.setText("Total: "+ String.format("%.2f", user.getTotal())+" AZN");

            ListView cartList = (ListView) view.findViewById(R.id.cartList);
            //AddressAdapter adapter = new AddressAdapter(getContext(), R.layout.address_item, user.getAddresses());
            CartAdapter adapter = new CartAdapter(getContext(), R.layout.cart_item, user.getCart(), tv);
